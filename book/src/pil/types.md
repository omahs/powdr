...
without an explicit type, their type has to be uniquely determined. If you want a symbol to have
a generic type, that generic type has to be explicitly specified.

Declarations of generic symbols can constraint type variables to satisfy certain bounds.
Currently, only built-in bounds or traits are supported (see the next section).
...
All other symbols use their proper type both for their value and for references to these symbols.
